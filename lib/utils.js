...
function gerarNumeroAleatorio(inicio, fim) {
    ...
    if(inicio < 0){ // 1º bug  (o inicio nao pode ser menor que zero)
        return -1;
    }
    ...
    const diff = fim - inicio; // 2º bug (implementação estava incorreta, gerava fora do intervalo)
...
function acharCaracter(tamanho, cadeia, caracter) { // 1º (decriçao da variavel nao era utlizado)
    ...
    for (let i = 0; i < cadeia.length; i++) {
        ...
        if (c === caracter) // 2º (a comparação era feita incorreta)
            return i;
    }

    return "caracter nao esta presente"; //(o retorno exibia 'caracter encontrado')
}
